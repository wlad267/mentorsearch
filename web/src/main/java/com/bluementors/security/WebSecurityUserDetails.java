...
public class WebSecurityUserDetails implements UserDetails {
    ...
    public static UserDetails create(User user){
        ...
        customUserDetails.authorities.add(new SimpleGrantedAuthority(AppRoles.Names.USER));

        if (user.isAdmin()) {
            customUserDetails.authorities.add(new SimpleGrantedAuthority(AppRoles.Names.ADMIN));
        }
        if (user.isMentor()){
            customUserDetails.authorities.add(new SimpleGrantedAuthority(AppRoles.Names.MENTOR));
        }
